...
const BackButton = ({ x, y, height, color }) => {
  const backButtonShadowProps = { dx: 0, dy: 0, blur: 4 };
  const OFFSET_PATH_X = (height - 20) / 2;
  const OFFSET_PATH_Y = (height - 17.03) / 2;
  return (
    <Group
      transform={[
        { translateX: x + (dim.BUTTON_WIDTH - height) + OFFSET_PATH_X },
        { translateY: y + OFFSET_PATH_Y },
      ]}
    >
      <Path
